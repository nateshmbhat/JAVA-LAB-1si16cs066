...
// Display the frame.
	...
	void fetchDatabase(int code){
		...
		try
		{
			...
			("jdbc:mysql://localhost/javalab_natesh?user=root&password=root123");
		...
		try{
			...
			String query1 =
			"SELECT * FROM Company WHERE emp_sal IN (SELECT MIN(emp_sal) FROM Company)";
			String query2 =
			"SELECT * FROM Company WHERE emp_sal IN (SELECT MAX(emp_sal) FROM Company)";
